...
export default function PersonalityProgressXP({
  current,
  total,
}: {
  current: number;
  ...
  const xp = current * 10;
  // Level is based on dimensions completed: Level 1-6 (one per dimension)
  const level = Math.min(Math.floor(current / 8) + 1, 6);
